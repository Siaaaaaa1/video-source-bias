...
def eval_retrieval(vid_txt_score_dicts, gt_txt_id2vid_id, id2data):
    ...
    def rearrange_matrix(score_matrix, row, col):
        rearranged_matrix = score_matrix[row, :]
        rearranged_matrix = rearranged_matrix[:, col]
        return rearranged_matrix
    def resort_score_matrix(score_matrix,gt_txt_id2vid_id,txt_idx2id,vid_id2idx):
        row_list = []
        col_list = []
        num_row = len(score_matrix)
        for idx in range(num_row):
            gt_col_id = gt_txt_id2vid_id[txt_idx2id[idx]]
            vidx = vid_id2idx[gt_col_id]
            row_list.append(idx)
            col_list.append(vidx)
        rematrix = rearrange_matrix(score_matrix,row_list,col_list)
        return rematrix
    save_torch_tensor(resort_score_matrix(score_matrix, gt_txt_id2vid_id, txt_idx2id, vid_id2idx),'sim_gen.npy')
    print(gt_txt_id2vid_id)
    print(txt_idx2id)
    print(vid_id2idx)
...
def start_inference(cfg):
    ...
    e2e_weights_path = "/data/gaohaowen/workspace/ALPRO-main/output/pretrain/alpro_pretrained_ckpt.pt"
    print(e2e_weights_path)
