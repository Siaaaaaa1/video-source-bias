...
def run():
    ...
    # 设置环境变量，禁用并行处理，以避免某些库的多线程问题
    os.environ['TOKENIZERS_PARALLELISM'] = "false"
    # TODO: 需要改进视觉化工具的创建,如果配置中指定了视觉化工具的类型，则初始化视觉化工具,如果没有指定视觉化工具，则设置为None
    ...
    # 构建分词器（tokenizer），用于文本预处理build tokenizer
    tokenizer = transformers.AutoTokenizer.from_pretrained(config['arch']['args']['text_params']['model'],
                                                           TOKENIZERS_PARALLELISM=False)

    # 初始化数据加载器（data_loader），用于训练和验证数据的加载setup data_loader instances
    ...
    # 构建模型架构，并打印模型结构到控制台build model architecture, then print to console
    model = config.initialize('arch', module_arch)
    logger.info(model)

    # 获取损失函数和评估指标的函数句柄 get function handles of loss and metrics
    loss = config.initialize(name="loss", module=module_loss)
    metrics = [getattr(module_metric, met) for met in config['metrics']]
    # 构建优化器，用于模型参数的更新build optimizer, learning rate scheduler. delete every lines containing lr_scheduler for disabling scheduler
    ...
    # 根据配置决定是否使用Neptune进行实验跟踪        
    if config['trainer']['neptune']:
        writer = ex
    else:
        writer = None
    # 初始化训练器（Trainer），并开始训练过程
