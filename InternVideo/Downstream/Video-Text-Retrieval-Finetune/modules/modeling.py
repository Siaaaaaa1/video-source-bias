...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    ...
    def forward(self, 
                input_ids, token_type_ids, attention_mask, video, video_mask, sample_rank, alpha):
        import random
        import numpy as np
        def sample_and_average(arr, prob):
            # 将数组中所有小于0的数都改成0
            arr = [max(0, x) for x in arr]
            # 初始化sum为0
            sum_val = 0
            # 以概率prob采样数组中的元素，并累加到sum中
            for value in arr:
                if random.random() < prob:
                    sum_val += value
            # 将sum除以数组长度并返回
            return sum_val / len(arr) if len(arr) > 0 else 0
        def cosine_similarity(vector_a, vector_b):
            """
            计算两个向量的余弦相似度。

            参数:
            vector_a (list 或 numpy.ndarray): 第一个向量。
            vector_b (list 或 numpy.ndarray): 第二个向量。

            返回:
            float: 两个向量的余弦相似度。
            """
            if isinstance(vector_a, torch.Tensor):
                vector_a = vector_a.cpu().detach()
            if isinstance(vector_b, torch.Tensor):
                vector_b = vector_b.cpu().detach()
            ...
            if norm_a == 0 or norm_b == 0:
                return 0  # 如果任一向量的模为0，则返回0，避免除以0的错误
            else:
                return dot_product / (norm_a * norm_b)
        # 将输入的tensor重塑为适合模型的尺寸
        ...
        sequence_output, visual_output = self.get_sequence_visual_output(
            input_ids, token_type_ids, attention_mask, video, video_mask, shaped=True, video_frame=video_frame
        )
        # print(sequence_output.shape)
        # print(visual_output.shape)
        split_size = sequence_output.size(0) // 2

        # 使用索引切分张量
        sequence_output_real = sequence_output[:split_size]
        sequence_output_ai = sequence_output[split_size:]
        visual_output_real = visual_output[:split_size]
        visual_output_ai = visual_output[split_size:]
        attention_mask_real = attention_mask[:split_size]
        attention_mask_ai = attention_mask[split_size:]
        video_mask_real = video_mask[:split_size]
        video_mask_ai = video_mask[split_size:]
        # 如果模型处于训练模式
        if self.training:
            loss = 0.
            cdcr_loss = 0.
            # 如果模型配置了特定的交互方式和CDCR损失
            if self.wti_interaction != 'no' and self.cdcr:
                # 获取相似性矩阵和CDCR损失
                sim_matrix, _, cdcr_loss = self.get_similarity_logits(
                    sequence_output_real, visual_output_real, attention_mask, video_mask, shaped=True, loose_type=self.loose_type
                )
            else:
                # go this
                # 否则只获取相似性矩阵
                sim_matrix_ai, *_tmp = self.get_similarity_logits(
                    sequence_output_ai, visual_output_ai, attention_mask_ai, video_mask_ai, shaped=True, loose_type=self.loose_type
                )
                sim_matrix_real, *_tmp = self.get_similarity_logits(
                    sequence_output_real, visual_output_real, attention_mask_real, video_mask_real, shaped=True, loose_type=self.loose_type
                )
            # print(sim_matrix.shape)
            # torch.set_printoptions(threshold=10_000, edgeitems=3, linewidth=150)
            # print(sim_matrix)
            # #org;
            # sim_sub = (sim_matrix_ai - sim_matrix_real)
            # sim_diag = torch.diagonal(sim_sub)
            # sim_diag = torch.clamp_min(sim_diag,0)
            # loss_2 = sim_diag.sum() * sample_rank # sample_rank 代表采样率50%、60%...100%
            ...
            # 计算抽取的元素数量
            num_elements = int(len(sim_diag) * sample_rank)

            # 随机抽取对应比例的tensor
            indices = torch.randperm(sim_diag.size(0))[:num_elements]
            selected_sim_diag = sim_diag[indices]

            loss_2 = selected_sim_diag.sum() # sample_rank 代表采样率50%、60%...100%
            
            # # 假设我们有一个tensor
            # tensor = torch.randn(16)  # 随机生成一个包含100个元素的tensor

            # # 步骤1: 创建一个随机布尔掩码
            # mask = torch.rand(tensor.size(0)) <= sample_rank  # 生成一个随机数tensor，小于0.5的为True
            # # 步骤2: 使用掩码选择tensor中的元素
            # selected_elements = sim_diag[mask]
            
            # 步骤3: 计算被选中元素的和
            
            # loss_2.requires_grad = False
            # for vc, vo, vg in zip(sequence_output,visual_output,visual_output_gen):
            #     sim_sub_array.append(
            #         cosine_similarity(vc.squeeze(0), vg.squeeze(0))-
            #         cosine_similarity(vc.squeeze(0), vo.squeeze(0)))
            # loss_2 = sample_and_average(sim_sub_array,sample_rank)
            # 计算相似性损失
            sim_loss = 0
            sim_loss1 = self.loss_fct(sim_matrix_real)
            sim_loss2 = self.loss_fct(sim_matrix_real.T)
            loss1 = (sim_loss1 + sim_loss2) / 2
            loss2 = loss_2 * alpha
            sim_loss = loss1
            sim_loss += loss2
            loss += sim_loss
            logger.info("Loss: %f, Loss1: %f, Loss2:%f",loss,((sim_loss1 + sim_loss2)/2).item(),(loss_2 * alpha).item())
            # import pandas as pd
            ...
            # global logger
            # if args.rank == 0:
            #     logger.info("Loss1: %f,Loss2:%f",((sim_loss1 + sim_loss2)/2).item(),(loss_2 * alpha).item())
            # save_loss1.append(((sim_loss1 + sim_loss2)/2).item())
            # save_loss2.append((loss_2 * alpha).item())
            # df = pd.DataFrame({'Value': save_loss1})
            # df2 = pd.DataFrame({'Value': save_loss2})

            # # 使用plotly创建折线图
            # fig = px.line(df, x=df.index, y='Value')
            # fig2 = px.line(df2, x=df2.index, y='Value')
            # # 给图表添加标题
            # fig.update_layout(title='Loss1')
            # fig2.update_layout(title='Loss2')
            # fig.write_image('Loss1.png')
            # fig2.write_image('Loss2.png')
            # 初始：
            # sim_loss = (sim_loss1 + sim_loss2) / 2
            # loss += sim_loss
            # 如果启用了CDCR损失，则将其加入总损失
            if self.cdcr:
                loss += self.cdcr_lambda * cdcr_loss
            return loss  # 返回损失值
        else:
            return None  # 如果不是训练模式，则返回None

    # 下面的三个函数用于获取文本序列输出、视频输出和它们的组合输出。
    # 这些函数首先会检查输入数据的形状是否已经符合要求（即是否已经"shaped"），
    # 如果没有，则会对数据进行重塑和必要的转换。
