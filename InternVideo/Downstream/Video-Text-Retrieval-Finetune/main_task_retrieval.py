...
def save_torch_tensor(tensor, file_path):
    # 确保Tensor在CPU上
    if tensor.is_cuda:
        tensor = tensor.cpu()
...
def save_to_txt(input_data, file_name):
    # 检查输入数据的类型
    if isinstance(input_data, (list, np.ndarray, torch.Tensor)):
        # 将输入数据转换为列表
        if isinstance(input_data, torch.Tensor):
            data_list = input_data.tolist()
        ...
        print("Input data is not a list, numpy array, or tensor.")
...
def get_args(description='CLIP4Clip on Retrieval Task'):
    ...
    parser.add_argument('--features_path', type=str, default='/data/gaohaowen/workspace/InternVideo-main/InternVideo1/Dataset/msrvtt/videos', help='feature path')
...
def prep_optimizer(args, model, num_train_optimization_steps, device, n_gpu, local_rank, coef_lr=1.):
    ...
    scheduler = None
    # optimizer = Adam(optimizer_grouped_parameters, lr=args.lr, weight_decay=weight_decay)
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, train_dataloader_gen, sample_rank, alpha=100, local_rank=0):
    global logger
    ...
    for step, (batch,batch_gen) in enumerate(zip(train_dataloader,train_dataloader_gen)):
        if n_gpu == 1:
            # multi-gpu does scattering it-self
            batch = tuple(t.to(device=device, non_blocking=True) for t in batch)
        input_ids, input_mask, segment_ids, video, video_mask = batch
        input_ids_gen, input_mask_gen, segment_ids_gen, video_gen, video_mask_gen = batch_gen
        input_ids = torch.cat((input_ids, input_ids_gen), dim=0)
        input_mask = torch.cat((input_mask, input_mask_gen), dim=0)
        segment_ids = torch.cat((segment_ids, segment_ids_gen), dim=0)
        video = torch.cat((video, video_gen), dim=0)
        video_mask = torch.cat((video_mask, video_mask_gen), dim=0)
        ...
        loss = model(
            input_ids, segment_ids, input_mask, video, video_mask, sample_rank, alpha)
        ...
        # dot.render(filename='/data/gaohaowen/workspace/InternVideo-main/InternVideo1/Downstream/Video-Text-Retrieval-Finetune/simple_model.pdf', view=False, format='pdf')
        # print("!!!!!!!")
        loss.backward()
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % log_step == 0 and args.rank == 0:\
                ...
                # Adam
                # logger.info("Epoch: %d/%s, Step: %d/%d, Lr: %f, Loss: %f, Time/step: %f", epoch + 1,
                #             args.epochs, step + 1,
                #             len(train_dataloader), optimizer.param_groups[0]['lr'],
                #             float(loss),
                #             (time.time() - start_time) / (log_step * args.gradient_accumulation_steps))
                start_time = time.time()
...
def eval_epoch(args, model, test_dataloader, device, n_gpu):
    ...
    if multi_sentence_:
        ...
        print("****************************************")
        save_numpy_array(sim_matrix,'sim_gen.npy')
        save_numpy_array(sim_matrix_dsl,'sim_dsl_gen.npy')
        print("****************************************")
...
def main():
    ...
    if args.do_train:
        ...
        args_gen.data_path="/data/gaohaowen/workspace/InternVideo-main/InternVideo1/Dataset/msrvtt_train_new/MSRVTT_data_gen.json"
        args_gen.train_csv="/data/gaohaowen/workspace/InternVideo-main/InternVideo1/Dataset/msrvtt_train_new/MSRVTT_train_gen.9k.csv"
        train_gen_dataloader, train_gen_length = DATALOADER_DICT[args_gen.datatype]["train_gen"](args_gen, tokenizer)
        ...
        print("train_gen_length",train_gen_length)
        # for batchA, batchB in zip(train_dataloader, train_gen_dataloader):
        #     # 在这里处理你的数据
        #     dataA = batchA[0]
        #     dataB = batchB[0]
        #     print(f"Data from DataLoader A: {dataA}")
        #     print(f"Data from DataLoader B: {dataB}")
        #     input()
        ...
        for epoch in range(resumed_epoch, args.epochs):
            ...
            sample_rank = 0.5 #去偏模型混合比例
            alpha = 0.002
            tr_loss, global_step = train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer,
                                               scheduler, global_step, train_gen_dataloader,sample_rank,
                                               alpha,
