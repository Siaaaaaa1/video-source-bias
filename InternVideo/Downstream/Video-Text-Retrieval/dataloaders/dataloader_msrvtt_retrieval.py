...
class MSRVTT_DataLoader(Dataset):
    ...
    def _get_rawvideo_dec(self, choice_video_ids, s=None, e=None):
        ...
        video = np.zeros((len(choice_video_ids), self.max_frames, 1, 3,
                          self.image_resolution, self.image_resolution), dtype=float)
        ...
        for i, video_id in enumerate(choice_video_ids):
            ...
            if num_frames > 0:
                ...
                if len(all_pos) > self.max_frames:
                    ...
                    # 随机
                    # sample_pos = np.random.choice(all_pos, size=self.max_frames, replace=False)
                    # 20%处
                    # 计算all_pos长度的20%, 计算第20%的位置, 提取第20%位置的元素
                    # all_pos_length = len(all_pos)
                    # index_20_percent = int(0.5 * all_pos_length)
                    # sample_pos = all_pos[index_20_percent:index_20_percent + 1]
                else:
                    sample_pos = all_pos
            ...
            if len(raw_video_data.shape) > 3:
                ...
                if self.max_frames < raw_video_slice.shape[0]:
                    if self.slice_framepos == 0:
                        ...
                        sample_indx = np.random.choice(raw_video_slice.shape[0], size=self.max_frames, replace=False)
                        video_slice = raw_video_slice[sample_indx, ...]
                else:
