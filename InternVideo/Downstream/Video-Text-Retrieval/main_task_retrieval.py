...
def _run_on_single_gpu(model, batch_list_t, batch_list_v, batch_sequence_output_list, batch_visual_output_list):
    ...
    def save_list_to_txt(filename, data):
        with open(filename, 'w') as file:  # 使用文本写入模式
            for item in data:
                file.write("%s\n" % item)  # 每个元素占一行

    # # 从文本文件读取列表
    def read_list_from_txt(filename):
        with open(filename, 'r') as file:  # 使用文本读取模式
            return [line.strip() for line in file]  # 去除每行末尾的换行符
    
    # 保存视频向量文件
    list_video = []
    list_caption = []
    for idx, b in enumerate(batch_visual_output_list):
        print(b.size())
        print(type(b))
        tmp_vis_list = b.squeeze()
        for row in tmp_vis_list:
            list_video.append(row)
    save_list_to_txt("/data/gaohaowen/workspace/InternVideo-main/InternVideo1/vector_output_time/vision_vector_eval_opensora_fusion_gen.txt",list_video)
    for idx, b in enumerate(batch_sequence_output_list):
        print(b.size())
        print(type(b))
        tmp_seq_list = b.squeeze()
        for row in tmp_seq_list:
            list_caption.append(row)
    save_list_to_txt("/data/gaohaowen/workspace/InternVideo-main/InternVideo1/vector_output_time/caption_vector_eval_opensora_fusion_gen.txt",list_caption)

    # 保存文本向量文件
    # for idx, b in enumerate(batch_list_t):
    #     visual_output = batch_visual_output_list[idx]
    #     print(visual_output)
    sim_matrix = []
